<template>
  <div class="mt-16 w-10/12 mx-auto">
    ...
    <form @submit.prevent="addRoom" class="mt-4 bg-gray-300 w-full p-4" v-if="showForm">
      <div class="flex flex-col">
        <h2 class="text-center py-2 font-extrabold text-2xl">Form New Room</h2>
        <input type="text" v-model="formRoom.name" placeholder="Room Name" class="input-user mb-2" />
        <input
          type="text"
    ...
    <form @submit.prevent="doEditRoom" class="mt-4 bg-gray-300 w-full p-4" v-if="editForm">
      <div class="flex flex-col">
        <h2 class="text-center py-2 font-extrabold text-2xl">Form Edit Room</h2>
        <input type="text" v-model="editRoom.name" placeholder="Room Name" class="input-user mb-2" />
        <input
          type="text"
    ...
    <table class="table-auto mt-4 w-full">
      ...
      <tbody>
        <tr v-for="(room, i) in rooms" :key="room.id" :class="[i%2!==0 ? 'bg-gray-300' : '']">
          ...
          <td class="table-col flex justify-between">
            <button @click="setEdit(room.id)" class="btn bg-orange-400">Edit</button>
            <button @click="deleteRoom(room.id)" class="btn bg-red-400">Delete</button>
          </td>
        </tr>
      </tbody>
    </table>
